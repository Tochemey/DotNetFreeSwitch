...
namespace DotNetFreeSwitch.Messages
{
   ...
   public class Message
   {
      ...
      {
         return BodyLines.Select(HeaderParser.SplitHeader)
      ...
      {
         foreach (var bodyParts in BodyLines.Select(HeaderParser.SplitHeader)
      ...
      public Dictionary<string, string> ParseBodyLines()
      {
         ...
         {
            var parsedLines = HeaderParser.SplitHeader(bodyLine);
            ...
            switch (parsedLines.Length)
            {
               case 1:
                  ...
                  break;
               default:
                  if (!resp.ContainsKey(parsedLines[0]))
                  {
