...
namespace ModFreeSwitch.Messages
{
    ...
    public class EslMessage
    {
        ...
        {
            return HasHeader(EslHeaders.ContentType) ? Headers[EslHeaders.ContentType] : string.Empty;
        }
        ...
        {
            return BodyLines.Select(EslHeaderParser.SplitHeader).Any(bodyParts => bodyParts[0].Equals(EslHeaders.ContentLength));
        }
        ...
        {
            foreach (var bodyParts in BodyLines.Select(EslHeaderParser.SplitHeader).Where(bodyParts => bodyParts[0].Equals(EslHeaders.ContentLength)))
            {
                int len;
